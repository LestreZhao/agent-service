...
# task_files_json_tool removed - reporter now uses direct LLM generation

from .llm import get_llm_by_provider
from src.config.agents import AGENT_LLM_MAP

# Create agents using configured LLM providers
research_agent = create_react_agent(
    get_llm_by_provider(AGENT_LLM_MAP["researcher"]),
    tools=[tavily_tool, crawl_tool],
    prompt=lambda state: apply_prompt_template("researcher", state),
)

coder_agent = create_react_agent(
    get_llm_by_provider(AGENT_LLM_MAP["coder"]),
...
db_analyst_agent = create_react_agent(
    get_llm_by_provider(AGENT_LLM_MAP["db_analyst"]),
...
document_parser_agent = create_react_agent(
    get_llm_by_provider(AGENT_LLM_MAP["document_parser"]),
...
reporter_agent = create_react_agent(
    get_llm_by_provider(AGENT_LLM_MAP["reporter"]),
...
chart_generator_agent = create_react_agent(
    get_llm_by_provider(AGENT_LLM_MAP["chart_generator"]),
    tools=[],  # No tools needed - direct LLM generation
